...
const HomePage = () => {
  ...
  const fetchVideos = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/video/tags/${q}`);
  ...
  return (
    <Sidebar>
      <div className="container mx-auto bg-gray-900 text-white">
        {/* Categories */}
        <h1 className="text-4xl  text-white mb-0 flex items-center">
  <FaHashtag className="mr-0 text-indigo-500 text-3xl" />
  <span className="text-3xl text-indigo-500  font-bold">{`${q}`}</span>
</h1>
<h3 className="ml-2 mb-1 text-gray-300">
  &bull;{`${videos.length} Videos`}
</h3>
