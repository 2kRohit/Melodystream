...
const HomePage = () => {
  ...
  const fetchVideos = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/video/category/${category}`);
        ...
        <div className="flex justify-center mt-0">
          <div className="flex space-x-4">
            {categories.map((category) => (
              ...
              <div
                key={category._id}
                className="bg-gray-400 text-black p-2 rounded-lg cursor-pointer hover:bg-gray-700 transition duration-200"
              >
