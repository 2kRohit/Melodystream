        ...
        <h2 className="text-center text-2xl text-blue-500 italic p-1 mb-0 mt-2 font-semibold"> Reports</h2>
          ...
          {filteredreports.length === 0 ? (
            ...
            <table className="w-full bg-gray-900 rounded-lg">
              ...
              <>
              <img
               onClick={()=>{navigate(`/admin/userprofile?uId=${report.user._id}`)}}
...
<div className={`${view?'':'hidden'}`}>
    ...
    <img
    onClick={()=>{navigate(`/admin/userprofile?uId=${comment.user._id}`)}}
      className="w-8 h-8 rounded-full mr-2 cursor-pointer"
      src={`http://localhost:8000/uploads/profile/${comment.user.profilePicture}`}
      alt="Profile"
    />
  ) : (
    <FaUserCircle className="w-8 h-8 text-gray-500 mr-2 cursor-pointer" onClick={()=>{navigate(`/admin/userprofile?uId=${comment.user._id}`)}}/>
  )}
                        <div>
