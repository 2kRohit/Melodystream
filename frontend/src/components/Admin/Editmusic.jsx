...
const UploadMusic = () => {
  ...
  const fetchMusic = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/admin/editmusic/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isValid) {
      ...
      try {
        ...
        const response = await axios.post(`http://localhost:8000/api/admin/changemusic/${id}`, data);
  ...
  return (
    <Sidebar>
      <div className="max-w-lg mx-auto bg-gray-900 p-12 shadow-2xl rounded-lg border-4 border-gray-600 mt-0">
        <h2 className="text-2xl font-semibold mb-4 -mt-5 text-blue-500 text-center italic">Edit Music</h2>
