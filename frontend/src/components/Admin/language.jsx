...
const CategoryForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
     ...
     const res= await axios.post('http://localhost:8000/api/music/addlanguage', { name });
     if (res.status===204){
      ...
      alert('Failed to insert language. Please try again.');
    }
  };
  ...
  const handleDeleteCategory = async (categoryId) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://localhost:8000/api/music/language/${categoryId}`);
        ...
        <div className="w-full max-w-lg border-4 border-gray-600 mt-5">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl text-blue-500 italic font-semibold flex items-center mx-auto mt-4">
              <span className="mr-2 ml-24 mb-2 mt-4">Language</span>
          ...
          <div className="flex items-center bg-gray-900 rounded-md ">
            ...
            <table className="w-full bg-gray-900 rounded-lg text-center">
              <thead>
                <tr>
                  <th className="p-2 text-white text-center">Language</th>
