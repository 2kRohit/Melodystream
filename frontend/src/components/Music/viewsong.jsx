...
useEffect(() => {
  ...
  const fetchMusic = async () => {
    try {
        if(type==='artist'){
      const response = await axios.get(`http://localhost:8000/api/music/getmusicbyartist/${value}`);
      setTrackList(response.data);
        }
        else if(type==='category'){
            const response = await axios.get(`http://localhost:8000/api/music/getmusicbycategory/${value}`);
            setTrackList(response.data);
              }
              else if(type==='language'){
                const response = await axios.get(`http://localhost:8000/api/music/getmusicbylanguage/${value}`);
                setTrackList(response.data);
                  }
                 else if(type==='mood'){
                    const response = await axios.get(`http://localhost:8000/api/music/getmusicbymood/${value}`);
                    setTrackList(response.data);
                      }
...
if(trackList.length===0) return(<Sidebar><h1 className='text-3xl text-gray-600 text-center mx-auto align-middle'>No songs available...</h1></Sidebar>)
  return (
    <Sidebar>
        <h1 className='text-3xl text-blue-500 text-center items-center mx-auto font-semibold capitalize'>{type} :<span className='text-white text-center'>{value}</span></h1>
    ...
    <tbody>
      {filteredTracks.map((track, index) => (
        ...
        >
          ...
          <td className="py-4 px-6 text-white">{track.category}</td>
          <td className="py-4 px-6 text-white">{track.mood}</td>
          <td className="py-4 px-6 text-white">{track.language}</td>
              ...
              {showPlaylist && selectedTrackId === track._id && (
                  ...
                  <ul className="text-white px-6 py-4">
                    {playlist.map((p) => (
                      <li
                        key={p.id}
                        className="cursor-pointer hover:border-b py-2 transition duration-300 flex items-center"
